...
void main() {
  ...
  group('AuthBloc test', () {
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.login(any(), any())).thenAnswer((_) => Future.value(const Right(true)));
        return AuthBloc(mockAuthRepository);
      },
      act: (authBloc) => authBloc.add(const AuthEvent.loginStarted(email: 'email', password: 'password')),
      expect: () => [
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.login(any(), any())).thenAnswer((_) => Future.value(const Right(false)));
        return AuthBloc(mockAuthRepository);
      },
      act: (authBloc) => authBloc.add(const AuthEvent.loginStarted(email: 'email', password: 'password')),
      expect: () => [
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.login(any(), any())).thenAnswer((_) => Future.value(Left(loginError)));
        return AuthBloc(mockAuthRepository);
      },
      act: (authBloc) => authBloc.add(const AuthEvent.loginStarted(email: 'email', password: 'password')),
      expect: () => [
